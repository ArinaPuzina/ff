...
struct AVLTree {
    ...
    //small right rotate when left in the left(or not<=) bigger
...
//Балансировка узла
    AVLTreeNode* balanceNode(AVLTreeNode* node) {
        ...
        //kевое вращение если баланс > 1 и вставка произошла в левое поддерево
    ...
    // Базовый случай: если узел не найден (дерево пусто или поддерево пусто), возвращаем nullptr
    if (node == nullptr) {
        return node;
    }

    // Находим узел для удаления, сравнивая значение с текущим узлом
    if (value < node->value) { // Если значение меньше текущего, идем в левое поддерево
        node->left = remove(node->left, value);
    } else if (value > node->value) { // Если значение больше текущего, идем в правое поддерево
        node->right = remove(node->right, value);
    } else { // Узел найден, начинаем процесс удаления
//У узла один потомок или он без потомков
        if (node->left == nullptr || node->right == nullptr) {
            // temp - потомoк
            AVLTreeNode* temp = node->left ? node->left : node->right;

            if (temp == nullptr) { 
                temp = node;// Временная переменная указывает на узел для удаления
                node = nullptr;   
            } else { //Если один потомок, замещаем удаляемый узел его потомком
                *node = *temp;    // Копируем данные
            }

            delete temp; // Удаляем узел
        } else {//У узла два потомка
            // Находим узел с минимальным значением в правом поддереве (наименьший элемент больше удаляемого)
            AVLTreeNode* temp = getMinValueNode(node->right);
            // Копируем значение из найденного узла в текущий узел
